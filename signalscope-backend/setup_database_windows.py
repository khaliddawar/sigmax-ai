"""
Setup SignalScope Database in Supabase (Windows-compatible version)
"""
import os
import sys
...
def test_supabase_connection():
    ...
    try:
        ...
        if not url or not key:
            print("[X] Supabase credentials not found in .env")
            return None
        ...
        try:
            ...
            if "relation" in str(e).lower() and "does not exist" in str(e).lower():
                print("[OK] Supabase connection successful!")
                print("[!] Tables don't exist yet (this is expected)")
            else:
                print(f"[!] Connection works but got error: {e}")
...
def test_openai_connection():
    ...
    try:
        ...
        if not api_key or api_key.startswith("sk-proj-your"):
            print("[X] OpenAI API key not found or is still placeholder")
            return False
        ...
        print(f"[X] OpenAI API test failed: {e}")
        if "api_key" in str(e).lower():
            print("TIP: Make sure your API key is valid and has credits")
        return False
...
def test_redis_connection():
    ...
    try:
        ...
        print(f"[OK] Redis connected! Total messages in queues: {total_messages}")
        return True
        
    except Exception as e:
        print(f"[X] Redis connection failed: {e}")
        print("TIP: Make sure Redis is running")
        return False
...
[OK] messages - Store captured messages
[OK] message_batches - Track processing batches  
[OK] reports - Store AI-generated reports
[OK] report_subscriptions - User subscriptions
[OK] ticker_stats - Ticker statistics
[OK] processing_jobs - Background job tracking
[OK] system_metrics - Performance metrics
