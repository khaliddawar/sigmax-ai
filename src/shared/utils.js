  ...
  try {
    ...
    if (typeof timestamp === 'string') {
      ...
      if (isNaN(date.getTime())) {
        // If it's just a time like "12:00:00", add today's date
        const today = new Date().toISOString().split('T')[0];
        const withDate = new Date(`${today}T${timestamp}`);
        if (isNaN(withDate.getTime())) {
          // If still invalid, return current time
          return new Date().toISOString();
        }
        return withDate.toISOString();
      }
    ...
    console.warn('formatTimestamp error:', error, 'for timestamp:', timestamp);
    return new Date().toISOString();
  }
}
