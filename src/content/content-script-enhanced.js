...
import MessageDeduplicator from '../shared/message-deduplicator.js';
...
// Add deduplication stats function
window.getDeduplicationStats = async function() {
  const stats = await messageDeduplicator.getStats();
  console.log('🔒 Message Deduplication Stats:', stats);
  return stats;
};

// Add function to clear deduplication cache (for testing)
window.clearDeduplicationCache = async function() {
  const cleared = await messageDeduplicator.clearAllHashes();
  console.log('🧹 Deduplication cache cleared:', cleared);
  return cleared;
};
...
window.testSignalScopeEnhanced = function() {
  ...
  if (platform === 'circle' || platform === 'webchat') {
    ...
    messages = document.querySelectorAll('.message-out, .message-in, .message, [class*="message"], .chatlist-chat');
...
class EnhancedContentScript {
  ...
  async initialize() {
    try {
      ...
      // Optional: set up a test bridge to accept page postMessage
      this.setupTestBridge();
  ...
  async startCapture() {
    try {
      ...
      // Initialize message deduplicator
      await messageDeduplicator.init();
  ...
  async handleCapturedMessage(message, context) {
    try {
      ...
      // Final content-side filter
      if (shouldSkipMessage(enrichedMessage)) {
        logger.debug('Skipping message by platform filter in enhanced script');
        return;
      }
      
      // Check for message deduplication (prevent page refresh duplicates)
      const isDuplicate = await messageDeduplicator.isMessageProcessed(enrichedMessage);
      if (isDuplicate) {
        logger.debug('🔄 Skipping duplicate message (already processed):', enrichedMessage.author, enrichedMessage.content?.substring(0, 50));
        return;
      }
      
      // Mark message as processed
      await messageDeduplicator.markMessageProcessed(enrichedMessage);
      
      // Send to background for webhook delivery
      console.log('📤 Sending to background - Author:', enrichedMessage.author, 'Content:', enrichedMessage.content?.substring(0, 50));
  ...
  // Test bridge: allow page context to trigger a test message via window.postMessage
  setupTestBridge() {
    try {
      window.addEventListener('message', (event) => {
        if (event.source !== window || !event.data) return;
        const { type, payload } = event.data;
        if (type === 'SIG_TEST_MESSAGE' && payload) {
          try {
            this.handleCapturedMessage(payload, { source: 'test', mode: this.hybridCapture?.captureMode || 'hybrid' });
          } catch (e) {
            console.error('SIG_TEST_MESSAGE failed:', e);
          }
        }
      });
    } catch (e) {
      console.error('setupTestBridge error:', e);
    }
  }
}
