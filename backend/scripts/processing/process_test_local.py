#!/usr/bin/env python3
"""
Process test transcript using local file storage
"""
import sys
import os
import json
...
# Force local storage by clearing Supabase environment variables
...
try:
    ...
    async def main():
        print("🎯 BPT Local Transcript Processing")
        print("=" * 50)
        ...
        # Create unique ID
        transcript_id = f'test_transcript_{int(time.time())}'
        
        # Initialize services with local storage
        transcript_processor = TranscriptProcessor()
        embedding_service = EmbeddingService()
        file_storage = FileStorageService(base_dir="data")
        ...
        try:
            ...
            print(f"\n🔄 Step 3: Storing locally...")
            
            # Create transcript record
            transcript_result = await file_storage.create_transcript_record(
                transcript_id=transcript_id,
                metadata=metadata
            )
            
            if transcript_result.get("success"):
                print(f"✅ Created transcript record")
            else:
                print(f"❌ Failed to create transcript record: {transcript_result.get('error')}")
                return
            
            # Store chunks and embeddings
            chunks_result = await file_storage.store_transcript_chunks(
                transcript_id=transcript_id,
                chunks=chunks,
            ...
            if chunks_result.get("success"):
                print(f"✅ Stored {chunks_result.get('count', 0)} chunks locally")
            else:
                print(f"❌ Failed to store chunks: {chunks_result.get('error')}")
                return
            ...
            if trades:
                ...
                for trade in trades:
                    await file_storage.create_trade_record(transcript_id, trade)
            ...
            chunks_dir = f"data/chunks/{transcript_id}"
            embeddings_dir = f"data/embeddings/{transcript_id}"
            transcript_file = f"data/transcripts/{transcript_id}.json"
            
            if os.path.exists(chunks_dir):
                chunk_count = len([f for f in os.listdir(chunks_dir) if f.endswith('.json')])
                print(f"✅ Chunks directory created with {chunk_count} files")
            else:
                print(f"❌ Chunks directory not created")
                
            if os.path.exists(embeddings_dir):
                embedding_count = len([f for f in os.listdir(embeddings_dir) if f.endswith('.json')])
                print(f"✅ Embeddings directory created with {embedding_count} files")
            else:
                print(f"❌ Embeddings directory not created")
                
            if os.path.exists(transcript_file):
                print(f"✅ Transcript metadata file created")
            else:
                print(f"❌ Transcript metadata file not created")
            
            print(f"\n🎯 Processing Complete!")
            print(f"📊 Transcript ID: {transcript_id}")
            print(f"🌐 Test the web interface at: http://localhost:8501")
            print(f"📊 Try asking questions about:")
            print(f"   • What did Patrick say about Bitcoin?")
            print(f"   • What did Patrick say about crude oil?") 
            print(f"   • What did Patrick say about S&P 500?")
            print(f"   • What did Patrick say about gold?")
